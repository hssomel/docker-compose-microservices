...
"""
    ...
    stage('Build & Publish Images to DockerHub') {
      steps {
        container('docker') {
          sh "docker build static -t gurkamal/static:latest"
          sh "docker build haproxy -t gurkamal/haproxy:latest"
          sh "docker build statistics -t gurkamal/statistics:latest"
    ...
    stage('Deploy to Kubernetes') {
      steps {
        container('docker') {
          sh 'export BASE64_AUTH_VALUE=\$(echo -n "\$ME_CONFIG_BASICAUTH_USERNAME:\$ME_CONFIG_BASICAUTH_PASSWORD" | base64)'
